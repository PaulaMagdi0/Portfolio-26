// ========================================================================
//  Hero3D — vanilla three.js, dropped into a React lifecycle.
//
//  Renders a wireframe icosahedron with amber vertex points, slowly rotates,
//  reacts to mouse-parallax on the camera. Reads theme colors from CSS vars
//  and updates live when the user flips the theme toggle.
//
//  Lazy: only mounts on non-touch, non-reduced-motion clients.
//  DPR clamped to 2. Cleanly disposes all GPU resources on unmount.
// ========================================================================
...
(function () {
  function Hero3D() {
    ...
    React.useEffect(() => {
      ...
      setEnabled(!!window.THREE && finePointer && !reduced);
    }, []);
    ...
    React.useEffect(() => {
      ...
      const readThemeColors = () => {
        ...
        const parse = (k) => {
          const parts = r.getPropertyValue(k).trim().split(/\s+/).map(Number);
          if (parts.length < 3 || parts.some(isNaN)) return new THREE.Color('#ededed');
          return new THREE.Color(`rgb(${parts[0]}, ${parts[1]}, ${parts[2]})`);
        };
        return { ink: parse('--ink'), amber: parse('--amber'), inkdim: parse('--inkdim') };
      };
      ...
      const dotsMat = new THREE.PointsMaterial({
        color: colors.amber, size: 0.07, transparent: true, opacity: 0.95,
      ...
      const innerMat = new THREE.LineBasicMaterial({
        color: colors.inkdim, transparent: true, opacity: 0.25,
      });
      ...
      const tick = () => {
        ...
        targetCamX = mouseX * 0.55;
        targetCamY = -mouseY * 0.55;
      ...
      return () => {
        ...
        dotsGeo.dispose(); dotsMat.dispose();
        renderer.dispose();
    ...
    return (
      <div ref={containerRef} className="absolute inset-0 pointer-events-none">
        {!enabled && (
          // Fallback for touch / reduced-motion: a static SVG primitive
          <svg viewBox="0 0 200 200" className="w-full h-full opacity-25" aria-hidden="true">
